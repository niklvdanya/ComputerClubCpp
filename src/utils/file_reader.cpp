...
FileReader::FileReader(const std::string& filename)
    : filename_(filename), currentLine_(0) {}
        ...
        switch (eventId) {
            case 1: {
                if (!(eventStream >> clientName)) {
                    throw FileReadException("Missing client name for event 1", currentLine_);
                }
                events.emplace_back(eventTime, models::EventType::CLIENT_ARRIVED, clientName);
                break;
            }
            case 2: {
                int tableNumber;
                ...
                events.emplace_back(eventTime, models::EventType::CLIENT_SAT, clientName, tableNumber);
                break;
            }
            case 3: {
                if (!(eventStream >> clientName)) {
                    throw FileReadException("Missing client name for event 3", currentLine_);
                }
                events.emplace_back(eventTime, models::EventType::CLIENT_WAITING, clientName);
                break;
            }
            case 4: {
                if (!(eventStream >> clientName)) {
                    throw FileReadException("Missing client name for event 4", currentLine_);
                }
...
int FileReader::validateAndParseInteger(const std::string& intStr, const std::string& errorMessage) {
    try {
