...
void ComputerClubController::closeClub() {
    ...
    for (const std::string& clientName : remainingClients) {
        ...
        if (client && client->isSeated()) {
            ...
            tableManager_->releaseTable(tableNumber, config_.getCloseTime(), config_.getHourlyRate());
        }
        
        eventManager_->addEvent(models::Event(
            config_.getCloseTime(),
            models::EventType::CLIENT_LEFT_END,
            clientName
        ));
    }
}
...
void ComputerClubController::addErrorEvent(const models::Time& time, const std::string& errorMessage) {
    eventManager_->addEvent(models::Event(
        time,
...
void ComputerClubController::assignTableToNextClient(const models::Time& time, int tableNumber) {
    ...
    if (clientManager_->hasClient(nextClientName)) {
        ...
        eventManager_->addEvent(models::Event(
            time,
            models::EventType::CLIENT_SAT_FROM_QUEUE,
            nextClientName,
            tableNumber
        ));
    }
